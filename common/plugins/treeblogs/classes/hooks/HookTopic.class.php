...
class PluginTreeblogs_HookTopic extends Hook
{
    ...
    public function RegisterHook()
    {
        ...
        $this->AddHook('template_topic_breadcrumbs_list_topic', 'TemplateTopicShowTopic', __CLASS__);
		$this->AddHook('template_topic_breadcrumbs_list_blog', 'TemplateTopicShowBlog', __CLASS__);
     ...
     * @param array $data
     */
    public function TemplateTopicShowTopic($aData)
    {
        $oTopic = $aData['topic'];
...
/*
		...
		*/
        $this->Viewer_Assign('aBlogsTree', $oBlogsTopic[0]);
        return $this->Viewer_Fetch(Plugin::GetTemplatePath('treeblogs') . 'actions/ActionTopic/crumbs.tpl');
    }
    public function TemplateTopicShowBlog($aData)
    {
        $oBlog = $aData['blog'];
		$oBlogsTree = array();
		$subBlog = $this->Blog_BuildBranch($oBlog->getId());
        array_push($oBlogsTree, $this->Blog_GetBlogsAdditionalData($subBlog));
		
       // $oBlogsTopic = $this->Topic_GetTopicBranches($oTopic);
 //print_r($oBlogsTree);
        $this->Viewer_Assign('aBlogsTree', $oBlogsTree[0]);
        return $this->Viewer_Fetch(Plugin::GetTemplatePath('treeblogs') . 'actions/ActionTopic/crumbs.tpl');
    }

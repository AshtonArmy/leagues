...
<form action="{$oTournament->getUrlFull()}match_insert/{$oMatch->getMatchId()}{if $team_result!=$myteam}/{$team_result}{/if}" onsubmit="return checkForm(this);"  method="post" id="form1" name="form1">
 
<p align="center">
			<span id="match_teams_teamplay"></span>
		</p>
		<p align="center">
			<input id="inputaway_teamplay" name="inputaway_teamplay" class="input-mini" {if $team_result==$oMatch->getAway()}onKeyUp="MyAdd(this.value)"{/if} type="text" SIZE="2" maxlength="2" {if $oMatchResult}value="{$oMatchResult->getAway()}"{/if}/>:<input id="inputhome_teamplay"  name="inputhome_teamplay" class="input-mini" type="text" {if $team_result==$oMatch->getHome()}onKeyUp="MyAdd(this.value)"{/if} SIZE="2" maxlength="2" {if $oMatchResult}value="{$oMatchResult->getHome()}"{/if}/> <span id="span_exist_o_teamplay" {if $oTournament->getExistO()==0}style="display:none;"{/if}>
